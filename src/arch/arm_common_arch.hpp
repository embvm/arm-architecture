 ...
 * ISB instruction should be used after CPSIE I. Figure 16 shows the use of the ISB instruction
 * after enabling interrupts to permit immediate recognition of the pending interrupt. If it is not
 * necessary to ensure that a pended interrupt will be recognized immediately before subsequent
 * operations, it is not necessary to insert a memory barrier instruction. Between two time critical
 * tasks, if you want to permit a pended interrupt to take place, you can use an ISB instruction as
 * follows:
 *
 * ```
 ...
 * __enable_irq();
 *
 * Source http://infocenter.arm.com/help/index.jsp?topic=/com.arm.doc.dai0321a/BIHBFEIB.html
 */
